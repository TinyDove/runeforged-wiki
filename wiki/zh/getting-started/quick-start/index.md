# 快速上手

本节用于建立 Runeforged 的系统框架。它不推荐流派，也不规定游玩顺序；每个页面只说明会看到什么、系统之间如何衔接，以及完整规则在哪里查询。

## 建议阅读顺序

| 顺序 | 主题 | 主要内容 |
| ---: | --- | --- |
| 1 | [武器与战技](weapons.md) | 四种新增武器、武器战技与副手攻击 |
| 2 | [世界与战斗](world-and-combat.md) | 怪物等级、元素、抗性与精英怪 |
| 3 | [装备锻造](equipment-forging.md) | 魔法/稀有装备、普通词条、材质升阶 |
| 4 | [特殊武器系统](special-weapons.md) | 符文诅咒、质变与特殊武器变体 |
| 5 | [诅咒与灌注](curses-and-infusion.md) | 诅咒类型、异化和灌注循环 |
| 6 | [传说与天界锻造](legendary-and-celestial.md) | 传说装备、普通词条 T0 与天界通货 |

## 数据查询

需要具体数值时，可以直接进入[数据库](../../reference/index.md)：

- [全武器词条表](../../reference/weapon-affixes.md)
- [全护甲词条表](../../reference/armor-affixes.md)
- [装备通货速查](../../reference/currencies.md)
- [玩家指令](../../reference/commands.md)

